...
import { LearningTextHelper, LearningText } from "@textactor/concept-domain";
...
export class MongoLearningText extends MongoItem<LearningText> {
  ...
  protected override beforeCreate(data: LearningText) {
    (<any>data)[EXPIRES_AT_FIELD] = new Date(data.expiresAt * 1000);

    return super.beforeCreate(data);
  }

  protected override beforeUpdate(data: RepositoryUpdateData<LearningText>) {
    data.set = data.set || {};
